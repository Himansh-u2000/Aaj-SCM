...
/**
 * Toast notification container — reads from Redux store.
 */
  ...
  return (
    <div className="fixed bottom-4 right-4 z-50 flex flex-col gap-2 max-w-sm">
      {toasts.map((toast) => (
        <div
          key={toast.id}
          className={`animate-slide-in-up rounded-lg px-4 py-3 shadow-dropdown flex items-center gap-3 text-sm font-medium
            ${toast.type === 'success' ? 'bg-green-600 text-white' : ''}
            ${toast.type === 'error' ? 'bg-red-600 text-white' : ''}
            ${toast.type === 'warning' ? 'bg-amber-500 text-white' : ''}
            ${toast.type === 'info' ? 'bg-secondary-800 text-white' : ''}
          `}
        >
          ...
          <button onClick={() => dispatch(removeToast(toast.id))} className="flex-shrink-0 opacity-70 hover:opacity-100 transition-opacity">
